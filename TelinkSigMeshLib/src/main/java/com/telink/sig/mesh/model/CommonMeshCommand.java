...
public class CommonMeshCommand {
    ...
    public final static int BIBOO_CMD_VOICE_ENABLE = 0x01;              //音箱开关
    public final static int BIBOO_CMD_RGB_SET = 0x02;             // RGB设置
    public final static int BIBOO_CMD_YELLOW_SET = 0x03;             //黄光设置
    public final static int BIBOO_CMD_CANDLE_SET = 0x04;             //-蜡烛光设置
    ...
    //设置闪烁模式cmd
    public final static int TWINKLE_CMD = 0x05;
    ...
    //同步RTC时间戳
    public final static int SYNC_TIME_CMD = 0X06;

    //时钟设置
    public final static int CLOCK_SET_CMD = 0X07;

    // 开关灯指令
    public final static int ON_OFF_SET = 0X08;
...
//        }
     ...
     * 闹钟设置的命令
     *
     * @param enable  是否开启闹钟 0关闭；1开启
     * @param mode    闹钟模式 0 一次；1每天重复
     * @param num     闹钟编号 0-5
     * @param hours   闹钟时针 0-23
     * @param minutes 闹钟分针 0-59
     * @param command 闹钟开启的命令 byte[]
     * @return 闹钟命令
     */
     ...
     * @return
     */
    public static byte[] getOnOffCommand(boolean isOn) {
        byte[] result = new byte[paramLength];
        int index = 0;
        result[index++] = defaultPre[0];
        result[index++] = defaultPre[1];
        result[index++] = (byte) ON_OFF_SET;
        if (isOn) {
            result[index++] = (byte) (1);
        } else {
            result[index++] = (byte) (0);
