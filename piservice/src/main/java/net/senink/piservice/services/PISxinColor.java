 ...
 */
public class PISxinColor extends PISBase {
    ...
    public static final byte PIS_CMD_CANDLE_SET = (byte)0x78;       //蜡烛灯开关
    /**
     * 消息类
     */
    public static final byte PIS_MSG_LIGHT_STATUS = (byte)0x30;  //当前灯的状态(可订阅)

    public PISxinColor(PIAddress addr, PIServiceInfo srvInfo){
        ...
        Commands.add(new PICommandProperty(PIS_CMD_CANDLE_SET,          // NextApp.tw
                "设置蜡烛灯的开关",
                "设置蜡烛灯的开关",
                PipaRequest.REQUEST_TYPE.REQUEST,
                true));
    ...
    public int getLightStatus(){
        if (this.ServiceType == PISBase.SERVICE_TYPE_GROUP){
            ...
            for (PISBase light : lights){
                try {
                    int rv1 = ((PISxinColor)light).getLightStatus();
                    rv = (rv > rv1)?rv:rv1;
                }
                catch (Exception e){
                    e.printStackTrace();
    ...
    public int getRedValue(){
        if (this.ServiceType == PISBase.SERVICE_TYPE_GROUP){
            ...
            for (PISBase light : lights){
                try {
                    int rv1 = ((PISxinColor) light).getRedValue();
                    rv = (rv > rv1) ? rv : rv1;
                }
                catch (Exception e){
                    e.printStackTrace();
    ...
    public int getGreenValue(){
        if (this.ServiceType == PISBase.SERVICE_TYPE_GROUP){
            ...
            for (PISBase light : lights){
                ...
                try {
                    int rv1 = ((PISxinColor)light).getGreenValue();
                    rv = (rv > rv1)?rv:rv1;
                }
                catch (Exception e){
                    e.printStackTrace();
    ...
    public int getBlueValue(){
        if (this.ServiceType == PISBase.SERVICE_TYPE_GROUP){
            ...
            for (PISBase light : lights){
                try {
                    int rv1 = ((PISxinColor)light).getBlueValue();
                    rv = (rv > rv1)?rv:rv1;
                }
                catch (Exception e){
                    e.printStackTrace();
    ...
    public int getWhiteValue(){
        if (this.ServiceType == PISBase.SERVICE_TYPE_GROUP){
            ...
            for (PISBase light : lights){
                try {
                    int rv1 = ((PISxinColor)light).getWhiteValue();
                    rv = (rv > rv1)?rv:rv1;
                }
                catch (Exception e){
                    e.printStackTrace();
    ...
    public byte[] getColorBytes(boolean needSave){
        ...
        databytes[4] = (byte)((needSave?1:0)<<4 | (this.getLightStatus() & 0xF));
...
//
//        if ((RedValue+GreenValue+BlueValue+WhiteValue) == 0)
//            setLightStatus(XINCOLOR_STATUS_OFF);
//        else
//            setLightStatus(XINCOLOR_STATUS_ON);
    }
    ...
    private transient XinColorBlinkData BlinkData;
    public XinColorBlinkData getBlinkData(){
        return BlinkData;
    }
    public void setBlinkData(byte[] blinkBytes){
        if (blinkBytes == null || blinkBytes.length != 7)
            return;
        if (BlinkData == null)
            BlinkData = new XinColorBlinkData();
    ...
    public void setBlinkData(XinColorBlinkData blink){
        if (blink == null)
            return;
        BlinkData = blink;
    }
    ...
    public static final int XINCOLOR_STATUS_OFF = 0;
    public static final int XINCOLOR_STATUS_ON  = 1;
    public static final int XINCOLOR_STATUS_BLINK = 2;
    ...
    public PipaRequest commitLightColor(int red, int green, int blue, int white, boolean needSave){
        byte[] colorBytes = new byte[5];

        colorBytes[0] = (byte)(red & 0xFF);
        colorBytes[1] = (byte)(green & 0xFF);
        colorBytes[2] = (byte)(blue & 0xFF);
        colorBytes[3] = (byte)(white & 0xFF);
        colorBytes[4] = (byte)(needSave?1:0);

        return new PipaRequest(this, PIS_CMD_LIGHT_RGBW_SET, colorBytes, 5, false);
    }

    public PipaRequest raiseLightRGBW(int redup, int greenup, int blueup, int whiteup, boolean needSave){
 ...
 *  降低颜色亮度
 */
    public PipaRequest reduceLightRGBW(int reddown, int greendown, int bluedown, int whitedown, boolean needSave){
    ...
    public PipaRequest commitCandleLight(boolean isOn){
        byte[] candleBytes = new byte[2];

        candleBytes[0] = (byte)0xAD;
        candleBytes[1] = (byte)(isOn?1:0);

        return new PipaRequest(this, PIS_CMD_CANDLE_SET, candleBytes, 2, false);
    }
